...
export function ColorPicker({ color, onChange }: ColorPickerProps) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-full bg-neutral-100 py-1.5 pl-3.5 pr-1.5">
      <input
        type="text"
        value={draft}
        ...
        className={`w-20 bg-transparent text-sm font-semibold uppercase tracking-wide outline-none ${
          isValid ? "text-black" : "text-neutral-400"
        }`}
      />
      <label className="relative h-6 w-6 shrink-0 cursor-pointer rounded-full border border-neutral-300">
        <span
